...
def lesson_text(lesson):
    ...
    lines = [f'{lesson.period}-juftlik | {lesson.start}–{lesson.end}', subject,
...
def split_escaped(blocks, limit=3900):
    ...
    for block in blocks:
        encoded = escape(block, quote=False)
...
def format_response(state, day: date, weekly=False, group='MR-86/25'):
    ...
    if state.error and not any(d.schedule for d in resolved):
        return [UNAVAILABLE]
    heading = 'Haftalik dars jadvali' if weekly else 'Bugungi dars jadvali'
    ...
    for result in resolved:
        ...
        if result.status == DayStatus.NO_VALID:
            ...
            lines.append('Darslar yo‘q.' if weekly else 'Bugun darslar yo‘q.')
        else:
